...
import {
  STATUS_LABEL, STATUS_ORDER, WHATSAPP_LABEL, formatPhone,
  type PhoneStatus, type WhatsappType,
...
function NumerosPage() {
  ...
  const numbersQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("phone_numbers")
        .select("id,phone_number,status,whatsapp_type,current_employee_id,carrier_id,employees!current_employee_id(name),carriers(name)")
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((n) => {
      ...
      if (employeeFilter !== "all" && employeeFilter !== "unassigned" && n.current_employee_id !== employeeFilter) return false;
      if (s) {
        const hay = `${n.phone_number} ${n.employees?.name ?? ""} ${n.carriers?.name ?? ""}`.toLowerCase();
      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Cadastre e gerencie os números telefônicos.</p>
          </div>
        ...
        {numbersQ.isLoading ? (
          <div className="grid gap-2">{[...Array(5)].map((_, i) => <Skeleton key={i} className="h-16" />)}</div>
        ) : filtered.length === 0 ? (
          <Card><CardContent className="p-8 text-center text-sm text-muted-foreground">Nenhum número encontrado.</CardContent></Card>
        ) : (
                  ...
                  <CardContent className="p-3 sm:p-4 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground truncate">
                        {n.employees?.name ?? "Sem responsável"} · {n.carriers?.name ?? "Sem operadora"} · {WHATSAPP_LABEL[n.whatsapp_type as WhatsappType]}
                      </div>
                    </div>
...
function NewNumberDialog({
  open, onOpenChange, employees, carriers, onCreated,
}: {
        ...
        <form onSubmit={handleSave} className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Input id="n-phone" required placeholder="(11) 99999-9999" value={phone} onChange={(e) => setPhone(e.target.value)} autoFocus />
            </div>
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Textarea id="n-obs" rows={2} value={observations} onChange={(e) => setObservations(e.target.value)} />
            </div>
          </div>
          <DialogFooter>
